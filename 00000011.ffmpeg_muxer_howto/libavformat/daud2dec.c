...
static int daud2_header(AVFormatContext *s) {
...
static int daud2_packet(AVFormatContext *s, AVPacket *pkt) {
    AVIOContext *pb = s->pb;
    int ret, size;
    if (avio_feof(pb))
        return AVERROR(EIO);
...
AVInputFormat ff_daud2_demuxer = {
    .name           = "daud2",
    .long_name      = NULL_IF_CONFIG_SMALL("D-Cinema audio"),
    .read_header    = daud2_header,
    .read_packet    = daud2_packet,
    .extensions     = "302,daud",
};
